...
public abstract class OldClose {
	...
	public static void close(OldClose c) {
		if (c == null) return;
		try { c.close(); } catch (Throwable t) { Mistake.log(t); } // Keep going to close the next object
	}

	/** true if c is null. */
	public static boolean no(OldClose c) { return c == null; }
	/** true if c exists. */
	public static boolean is(OldClose c) { return c != null; }
...
//	public static boolean going(Close c) { return c != null && !c.closed(); } //TODO choose a better name for this one, or all four of them
	...
	public static boolean done(OldClose c) { return c != null && c.closed(); }

	// See
	private static final Set<OldClose> list = new HashSet<OldClose>();
	private static synchronized void add(OldClose c) { list.add(c); }
	private static synchronized void remove(OldClose c) { list.remove(c); }
